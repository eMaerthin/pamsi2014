...
void Graf<TYP>::alg_Dijkstry(int start)
{
	...
	vector<CalePolaczenie> kolejka;
	int v, schowek;;

	for (i = 0; i < rozmiar; ++i){
		if (i != start) kolejka.push_back(CalePolaczenie(-1, i, INF_)); //-1 bo nie potrzebujemy wartosci pierwszego wierzcholka
	}

	while (!kolejka.empty()){
		posortuj_szybko_wektor(kolejka);

		v = kolejka.front().drugi_wierzcholek;
		kolejka.erase(kolejka.begin());

		for (i = 0; i < rozmiar; ++i){
			if (D[i] + graf_tablica[i][v] < D[v]){
				D[v] = D[i] + graf_tablica[i][v];
			}
		}

		for (i = 0; i < kolejka.size(); ++i){
			schowek = kolejka[i].drugi_wierzcholek;
			kolejka[i] = CalePolaczenie(-1, schowek, D[schowek]);
		}
	}
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
