...
int Game::getBestMoveValue()
{
	int bestMove = -1;
	//kiedy ruch wykonuje gracz 1 (kolka/poziomo)
	if(getPlayer() == 1) {
		for(int i = 2*_sizeBoard+1; i < (2*_sizeBoard+1)*(2*_sizeBoard+1) - 2*_sizeBoard+1; i++) {
			if(_fields[i] == 2 && _fields[i - (2*_sizeBoard+1)] == 0 && i > (2*_sizeBoard+1)) {
				if(i%(2*_sizeBoard+1) >= 2*_sizeBoard - 1) bestMove = natezenie();
				else if(_fields[i - 1] != 0 && _fields[i + 1] != 0) bestMove = natezenie();
				else bestMove = i - (2*_sizeBoard+1);
				break;
			}
			else if(_fields[i] == 2 && _fields[i + (2*_sizeBoard+1)] == 0) {
				if(i%(2*_sizeBoard+1) >= 2*_sizeBoard - 1) bestMove = natezenie();
				else if(_fields[i - 1] != 0 && _fields[i + 1] != 0) bestMove = natezenie();
				else bestMove = i + (2*_sizeBoard+1);
				break;
	...
	//kiedy ruch wykonuje gracz 2 (gwiazdki/pionowo)
	if(getPlayer() == 2) {
		for(int i = 2*_sizeBoard+1; i < (2*_sizeBoard+1)*(2*_sizeBoard+1) - 2*_sizeBoard+1; i++) {
			if(_fields[i] == 1 && _fields[i+1] == 0) {
				if(i%(2*_sizeBoard+1) >= 2*_sizeBoard -1) bestMove = natezenie();
				else if(_fields[i - (2*_sizeBoard+1)] != 0 && _fields[i + (2*_sizeBoard+1)] != 0) bestMove = natezenie();
				else bestMove = i + 1;
				break;
			}
			else if(_fields[i] == 1 && _fields[i - 1] == 0 && i%(2*_sizeBoard+1) != 0) {
				if(i%(2*_sizeBoard+1) >= 2*_sizeBoard - 1) bestMove = natezenie();
				else if(_fields[i - (2*_sizeBoard+1)] != 0 && _fields[i + (2*_sizeBoard+1)] != 0) bestMove = natezenie();
				else bestMove = i - 1;
				break;
...
int Game::natezenie() {
	int player = getPlayer();
	for(int i = 2*_sizeBoard; i < (2*_sizeBoard+1)*(2*_sizeBoard+1) - 2*_sizeBoard+1; i+=2) {
		if(_fields[i] == player && _fields[i+(2*_sizeBoard+1)*2 + 2] == player && _fields[i+(2*_sizeBoard+1) + 1] == 0) 
			return i+(2*_sizeBoard+1) + 1;
		if(_fields[i] == player && _fields[i+(2*_sizeBoard+1)*2 - 2] == player && _fields[i+(2*_sizeBoard+1) - 1] == 0) 
			return i+(2*_sizeBoard+1) - 1;
	}
	return -1;
