...
bool k_kopiec::Przywroc() {
	if (rozmiar < 1) { cout << "Kopiec jest pusty." << endl; return false; }
	if (is_heap(&(kopiec[0]), &(kopiec[rozmiar]))) { cout << "Wlasciwosci kopca sa zachowane." << endl; return true; }
	
	else {	while (!is_heap(&(kopiec[0]), &(kopiec[rozmiar]))) { make_heap(&(kopiec[0]), &(kopiec[rozmiar])); }
	cout << "Wlasciwosci kopca zostaly przywrocone." << endl; return true; }
}


void k_kopiec::Wyswietl() {
	if (rozmiar <= 0) { cout << endl << "Kopiec jest pusty!" << endl; }

	cout << endl << "\t\t\t\t" << kopiec[0] << endl << "\t\t\t      "; //szczyt kopca
	int poziom = 1; //kopca - wizualnie
	int ilosc = 2;
	for (int i = 0; i < rozmiar; i++) {
		if (poziom == 2) cout << "\t\t";
		if (poziom == 3) cout << "\t";
		if ((2 * i + 1) >= rozmiar) break; // jesli pierwszy nieparzysty element jest koncem kopca
		cout << kopiec[2 * i + 1]; //wyswietlenie nieparzystego

		if ((2 * i + 2) >= rozmiar) break; //jesli pierwszy parzysty element kopca jest koncem
		cout << " | " << kopiec[2 * i + 2] << "\t"; //wyswietlenie przerwy w wezle, parzystego elementu i przerwy przed nastepnym wezlem

		if (ilosc % (2 * i + 2) == 0) { poziom++; //jesli suma elementow na danym poziomie jest parzysta, zmieniamy poziom na nastepny
			ilosc += (poziom*poziom); // ilosc elementow na poziomie kopca narasta w tempie kwadratowym
			cout << endl; }
	}
cout << endl;
}
...
bool k_kopiec::Usun_szczyt() {
	if (rozmiar < 1) { cout << "Kopiec jest pusty!" << endl; return false; }
	rozmiar -= 1;
	TYP * temp= new TYP[(rozmiar)]; // mniejszy kopiec tymczasowy
	for (int i = 0; i < rozmiar; i++) temp[i] = kopiec[i + 1]; //przepisanie kopca do mniejszego
	free(kopiec); // zwalnianie pamieci
	kopiec = new int[rozmiar]; //tworzymy nowy kopiec
	for (int i = 0; i < rozmiar; i++) kopiec[i] = temp[i]; //zapisujemy nowy kopiec danymi z tympczasowego
